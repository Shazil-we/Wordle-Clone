...
let i = 0
let used_word:string[] = []
let Style_Array:string[] = []
let Style_Array2 = ''
let s = 0
let h = 0;
console.log(chalk.yellowBright("-------------------------------------------------------------------------------------------------") + chalk.greenBright("WORDLE") + chalk.yellowBright("----------------------------------------------------------------------------------------------"))
function Wordle_Function(letter: string) {
  if (
    letter == answer_char[0] ||
...
function Random_Answer_Generator(min:number , max:number)
{
  let i = Math.floor(Math.random() * (max - min) + min)
  return fiveLetterWords[i]
}
function checker(user_value:string){
...
for(let k=0; k<6; k++){
  ...
  if(user_input[k+1].length == 5){
    let input_char = [
      ...
      for(let j=0; j<5; j++){
        let test:string = Wordle_Function(input_char[j])
